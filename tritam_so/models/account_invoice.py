...
class AccountInvoiceLineInherit(models.Model):
    ...
    @api.depends('price_unit', 'discount', 'invoice_line_tax_ids', 'quantity',
                 ...
                 'invoice_id.date_invoice','x_discount')
    def _compute_price(self):
        currency = self.invoice_id and self.invoice_id.currency_id or None
        price = self.price_unit * (1 - (self.discount or 0.0) / 100.0)
        taxes = False
        discount = self.x_discount or 0
        if self.invoice_line_tax_ids:
            taxes = self.invoice_line_tax_ids.compute_all(price, currency, self.quantity, product=self.product_id,
                                                          partner=self.invoice_id.partner_id)
        self.price_subtotal = price_subtotal_signed = (taxes['total_excluded'] if taxes else self.quantity * price) - discount
